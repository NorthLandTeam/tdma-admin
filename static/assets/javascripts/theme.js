...
(function( $ ) {
	...
	var LockScreen = {
        ...
        //验证信息，若密码通过，则隐藏
				...
				$.ajax({
						type: "POST",
						url: basePath+"checkUserPassword",//url请求的地址
						cache: false,  //禁用缓存
						async: false,
						data: {
							"username":username,
		...
		buildTemplate: function( userinfo ) {
			return [
								...
								'<div class="panel-body">',
									'<form th:action="@{/checkLogin}" method="post">',
										'<div class="current-user text-center">',
											'<img id="LockUserPicture" src="{{picture}}" alt="{{username}}" class="img-circle user-image" />',
										...
										'<div class="form-group mb-lg">',
											'<div class="input-group input-group-icon">',
											  '<input id="user_name" name="user_name" type="hidden" value="{{username}}"/>',
												'<input id="pwd" name="pwd" type="password" class="form-control input-lg" placeholder="请输入密码" />',
				...
				.join( '' )
				.replace( /\{\{picture\}\}/g, userinfo.picture )
